...
本文按当前 `router.py`、`schema.py`、`service.py` 实现整理，更新日期：2026-09-22。
...
202 只表示已保存任务，不表示合成成功。模板不存在、媒体读取失败等后台错误通过任务结果返回。请求字段非法返回 422，服务配置不完整或服务正在停止返回 503。
