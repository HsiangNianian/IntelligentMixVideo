...
`proto/imv/remotion_typography/v1/remotion_typography.proto` 定义已验收字效版本和发布后固定视频资产的交换契约。
参数、文字和动画在 Remotion 工作区定稿；导入资产库后由透明、无声媒体与固定文字清单供模板和总线引用。
模板轨道沿用开始方式、开始位置与固定持续时间，只能从源零帧播放并裁掉尾部，不替换文字或重新计算动画。
当前仅生成不带透明通道的 `preview.mp4`，尚无资产导入、透明媒体发布或总线消费实现；协议中的交付形式须经实际合成验证。
