...
剩余预算较小时进一步缩减。优先覆盖文字稳定展示与动效边界；失败引用帧、Judge 明确请求的补充帧优先保留，
...
请求前以文本 UTF-8 字节数的一半及每图 2048 token 估算输入，按剩余总预算与角色预算预留输出空间，
不足时仅在本次请求中按完整旧交互组裁剪窗口，不改持久历史或拆散工具回执；最少保留最新一组。
剩余输出不足 512 token（单次配置更小时取该配置）则以 `input_budget_exhausted` 停止，尚未发送的请求不记调用成本。
该估算不是供应商分词或计费保证，响应后仍按实际 `total_tokens` 严格结算；不调整既有预算上限。
