...
test("prepares all client versions from tag while preserving dependencies", () => {
...
test("rejects MSI-incompatible versions even when manifests agree", () => {
  const result = validate("v256.1.0", (dir) => {
    for (const path of ["client/package.json", "client/src-tauri/tauri.conf.json", "client/src-tauri/Cargo.toml", "client/src-tauri/Cargo.lock"]) {
      const file = join(dir, path);
