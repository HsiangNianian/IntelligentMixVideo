...
import { textRoles, type Category, type Draft, type Editor, type EffectAsset, type TextRole } from "./model";
...
export function EffectAssets({ draft, catalog, textTarget, onTextTarget, onApply, onAsset, textEditor }: {
  draft: Draft;
  catalog: EffectAsset[];
  textTarget: TextRole;
  ...
  const editor = (isMotion || category === "flower") && textEditor ? textEditor : draft.editor;
...
export function AppliedEffects({ draft, catalog, selected, onSelect }: {
  draft: Draft; catalog: EffectAsset[]; selected: string | null; onSelect: (target: string) => void;
}) {
  if (draft.tracks) return <section aria-label="已添加特效" className="space-y-3 border-t bg-card p-4">
  ...
  return <section aria-label="已添加特效" className="space-y-3 border-t bg-card p-4">
    ...
    {!targets.length && <p className="text-sm text-muted-foreground">从左侧特效资产选择并添加效果。</p>}
  </section>;
}
