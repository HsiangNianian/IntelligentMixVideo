...
首页「设置」另提供模块配置表单，字段来自后端插件目录，值保存在当前客户端，和上述构建环境变量分开。当前有切片与 ASR，ASR 暂用于设置展示和本地保存，尚不影响转写业务；桌面保存本地 JSON（包含明文 API Key），浏览器仅当前页面内存保存。协议、插拔与联调步骤见 [模块设置说明](src/features/settings/README.md)。
