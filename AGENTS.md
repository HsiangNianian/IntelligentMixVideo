...
- `App.tsx` 挂载 `pages/HomePage.tsx`，左侧导航提供默认打开的「主页」、`features/templates/` 模板库和 `features/remotion_templates/` 字效工作区。主页通过 `TemplateHome` 按云端在上、本地在下展示模板，各库独立读取和重试，浏览器提示本地模板需要桌面客户端；重新进入主页刷新列表。选择模板携带环境和 ID 进入模板库，读取最新详情并复用保存/放弃/取消保护，跨库读取全部成功后才替换环境与草稿。底部设置入口组合环境信息与 `features/settings/` 动态插件表单；窄屏使用带无障碍名称的图标栏，切换设置同样保留工作区。聊天、任务编排、隔离 Player、参数编辑和 API 请求按职责分离。
