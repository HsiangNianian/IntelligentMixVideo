...
- Judge 接收原始请求、最新修改、成功基线与完整候选方案；方案只解释实现。失败项须引用实际要求并声明作用对象、观察与不匹配，宿主校验引用和结构化作用域，不能把无依据否决交给 Actor，也不能声称字符串检查能证明语义正确。检查灰底不属于模板内容。
- 模型读取代表帧，宿主保留全部 frame/probe 校验；Judge/Actor 通常最多接收 12/6 帧，预算较小时缩减，明确要求的补充帧与失败引用帧优先保留。模型帧号映射只对应实际发送图片，未展示的已有帧也可请求。请求前估算输入并预留输出，紧张时按完整旧工具组裁剪本次 Actor 请求，不修改持久窗口；实际 token 回执仍严格结算。私有审计记录图片数、输入估算和可用的 token 明细。
