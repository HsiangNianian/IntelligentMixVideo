...
- 模板保存结构中的 `tracks` 为必填数组，参数仅保存在 `tracks[].editor`。参数面板使用独立的 `EffectDraft` 临时视图。前端读取、云端 API/数据库、本地 Rust 文件存储及合成均要求对象格式，拒绝缺少 `tracks`、`tracks: null` 或携带顶层 `editor` 的模板，不提供旧格式转换。
