...
- `proto/imv/remotion_typography/v1/remotion_typography.proto` 定义已验收字效版本、固定透明视频资产及供模板轨道引用的资产身份；当前只是协议，尚未实现导入和合成。字效内容与参数在导入前定稿；模板轨道复用 `start_mode/start/duration`，Remotion 轨道只允许固定正时长、移动入点及从源零帧裁掉尾部，不使用 IMS 的文字动画缩短逻辑。
