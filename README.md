...
根目录 `buf.yaml` 管理 `proto/` 中的消息定义及兼容性检查，`buf.gen.yaml` 生成客户端 TypeScript、Rust 和服务端 Python 代码。IMS 模板生成代码位于各自的 `src/generated/imv/template/v1/`，Rust 位于 `client/src-tauri/src/generated/`；Remotion 字效资产协议生成代码位于客户端和服务端各自的 `src/generated/imv/remotion_typography/v1/`。修改协议后在仓库根目录运行 `buf lint`、`buf generate`，并提交生成结果；已有协议版本使用 `buf breaking --against <基线>` 检查兼容性。
