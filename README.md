...
首页左侧提供「模板库」「Remotion 字效」和底部「设置」导航，窄屏收为图标栏；普通模式的设置弹窗只展示客户端环境与后端服务地址；`IMV_DEBUG=true` 时按后端自动发现的插件目录生成模块导航，点击模块按钮查看对应表单，提供切片模型与 ASR 设置；ASR 当前仅用于展示和本地保存，尚未接入转写业务。Debug 配置仅保存在当前客户端，独立切片请求可携带使用；普通模式不读取或携带这些配置，不修改后端 `.env`；尚未接入视频合成或新增切片工作区。切换页面保留工作区草稿和订阅。存储与插拔说明见 [客户端设置](client/src/features/settings/README.md)。
