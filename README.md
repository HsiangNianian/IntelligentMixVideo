...
首页左侧提供「模板库」「Remotion 字效」和底部「设置」导航，窄屏收为图标栏；设置展示环境与字效服务地址，并按后端插件目录生成独立的横向模块导航，点击模块按钮查看对应表单，首期预置切片模型配置。配置仅保存在当前客户端，独立切片请求可携带使用，不修改后端 `.env`；尚未接入视频合成或新增切片工作区。切换页面保留工作区草稿和订阅。存储与插拔说明见 [客户端设置](client/src/features/settings/README.md)。
